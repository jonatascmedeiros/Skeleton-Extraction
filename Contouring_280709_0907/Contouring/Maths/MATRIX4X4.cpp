...
{
	return VECTOR3D(rhs.x+entries[3], rhs.y+entries[7], rhs.z+entries[11]);
}
...
void MATRIX4X4::SetRotationAxis(const float angle, const VECTOR3D & axis)
{
...
{
	float cr = cos( M_PI*angleX/180 );
	float sr = sinf( M_PI*angleX/180 );
	float cp = cosf( M_PI*angleY/180 );
	float sp = sinf( M_PI*angleY/180 );
	float cy = cosf( M_PI*angleZ/180 );
	float sy = sinf( M_PI*angleZ/180 );
